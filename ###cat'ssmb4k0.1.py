...
class Game:
    ...
    def update_game(self):
        ...
        for e in ld.enemies:
            ...
            if abs(e['x'] - p.x) < SCREEN_W + 100: # Only update near player
                ...
                # Wall turn-around
                # FIXED: Added bounds checking to prevent IndexError when enemies go off-map
        ...
        for f in p.fireballs[:]:
            ...
            # FIXED: Added bounds checking for fireballs
    ...
    def draw_game(self):
        ...
        if not p.dead:
            ...
            draw_mario(self.screen, p.x - cam, p.y, 'jump', 0, p.facing, p.big, p.fire)

        # Fireballs
        for f in p.fireballs:
             pygame.draw.circle(self.screen, C_FIREBALL, (int(f['x'] - cam), int(f['y'])), 6)
        ...
        # HUD
        hud_score = self.hud_font.render(f"MARIO  {p.score:06d}", True, C_HUD)
        hud_coins = self.hud_font.render(f"COINS  x{p.coins:02d}", True, C_HUD)
        hud_world = self.hud_font.render(f"WORLD  {self.world}-{self.level}", True, C_HUD)
        
        self.screen.blit(hud_score, (50, 20))
        self.screen.blit(hud_coins, (300, 20))
        self.screen.blit(hud_world, (500, 20))
